<template>
  <div :id="uploadId" class="fm-uplaod-container">
    <draggable v-model="fileList" class="drag-img-list" v-bind="{group: uploadId, ghostClass: 'ghost', animation: 200}" :no-transition-on-drag="true">
      <div v-for="(item) in fileList" :id="item.key" :key="item.key" :style="{width: width+'px', height: height+'px'}" :class="{uploading: item.status=='uploading', 'is-success': item.status=='success', 'is-diabled': disabled}" class="upload-file">
        <img :src="item.url">
        ...
        <div v-if="!disabled" class="uplaod-action" :style="{height: miniWidth / 4 + 'px'}">
          <i class="iconfont icon-tupianyulan" title="预览" :style="{'font-size': miniWidth/8+'px'}" @click="handlePreviewFile(item.key)"></i>
          <i v-if="isEdit" class="iconfont icon-sync1" title="替换" :style="{'font-size': miniWidth/8+'px'}" @click="handleEdit(item.key)"></i>
          <i v-if="isDelete && fileList.length > min" class="iconfont icon-delete" title="删除" :style="{'font-size': miniWidth/8+'px'}" @click="handleRemove(item.key)"></i>
        </div>
      </div>
    </draggable>

    <div v-show="(!isQiniu || (isQiniu && token)) && fileList.length < length" class="el-upload el-upload--picture-card" :class="{'is-disabled': disabled}" :style="{width: width+'px', height: height+'px'}" @click.self="handleAdd">
      <i class="el-icon-plus" :style="{fontSize:miniWidth/4+'px',marginTop: (-miniWidth/8)+'px', marginLeft: (-miniWidth/8)+'px'}" @click.self="handleAdd"></i>
